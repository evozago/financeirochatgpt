...
-- 0) Pré-requisitos suaves: garantir unique na chave da NFe, se a tabela existir
DO $$
BEGIN
  IF EXISTS (
...
-- 1) Tabela de vínculo NFe ↔ Parcela (1 NF pode ter N parcelas vinculadas)
...
-- 2.1) NFes pendentes de conciliação (não têm vínculo)
...
-- 2.2) NFes conciliadas com totais (soma do valor de parcelas vinculadas)
--     Assume colunas n.valor_total (ou n.valores->>'total') e p.valor_parcela
--     Ajuste o campo do total da NFe conforme sua tabela
...
SELECT 
  n.chave_acesso,
  n.emitente,       -- ajuste conforme colunas reais (json/texto)
  n.destinatario,   -- idem
  /* tente obter o total; troque se a sua tabela usa outro campo */
  COALESCE(CASE 
    WHEN n.valores IS NOT NULL THEN (n.valores->>'total')::numeric
    ELSE NULL
  END, NULL) AS total_nfe,
  s.total_parcelas,
  s.qtd_parcelas,
  (COALESCE(CASE WHEN n.valores IS NOT NULL THEN (n.valores->>'total')::numeric ELSE NULL END, 0) - COALESCE(s.total_parcelas,0)) AS diferenca
FROM public.nfe_data n
LEFT JOIN soma s ON s.chave_acesso = n.chave_acesso;

-- 3) Função: conciliar NFe
--    - Se conta_id for NULL e criar_conta = true → cria conta + 1 parcela (valor total da NFe) e vincula
--    - Se conta_id informado → vincula a primeira parcela "a_vencer" ou "vencida" que ainda não esteja vinculada
--    - Retorna: chave, conta_id usado/criado, parcela_id vinculada
...
BEGIN
  ...
  -- Tentar extrair total da NFe (ajuste o campo conforme seu schema)
  SELECT 
    CASE 
      WHEN valores IS NOT NULL THEN (valores->>'total')::numeric
      ELSE NULL
    END
  INTO v_total
  ...
  -- Criar conta/parcela se solicitado e as tabelas existirem
  ...
  ELSIF p_conta_id IS NOT NULL THEN
    ...
    -- pegar uma parcela não vinculada desta conta (status aberto)
    SELECT p.id
    INTO v_parcela
    ...
    WHERE p.conta_pagar_id = v_conta
      ...
      AND p.id NOT IN (SELECT parcela_id FROM public.nfe_parcela_link)  -- evita reaproveitar parcela já vinculada a outra NFe
  ...
  -- Vincular (idempotente pela unique)
...
-- 4) Função: desfazer conciliação (remove todos os vínculos da NFe)
...
-- 5) Função: listar NFes pendentes (atalho)
...
-- 6) RLS básico na tabela de vínculo (mesmo padrão que você já usa)
